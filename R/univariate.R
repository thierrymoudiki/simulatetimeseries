...
#' @param trend Character. "linear" or "quadratic".
#' @param seasonality Character. "none" or "sinusoidal".
#' @param distribution Character. "normal" or "student".
#' @param noise_sd Numeric. Standard deviation of noise.
...
#' @return time series object without NA values
#'
...
#' @return logical indicating if object is in list
#'
check_list_contains <- function(lst, obj) {
  for (item in lst) {
    if (!is.null(item) && identical(item, obj)) {
      return(TRUE)
    }
  }
  return(FALSE)
